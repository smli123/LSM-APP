...
public class DuerOSActivity extends TitledActivity implements OnClickListener, TextToSpeech.OnInitListener {
	private static String TAG = "DuerOSActivity";
	
	public static int SERVER_PORT = 8000;
	private final static String UDP_SCANPORT_COMMAND	= "ScanIP#";	// 扫描UDP端口号用的命令行语句
	
	private static TextView et_ip_address;
	private TextView tv_moduleid;
	private TextView tv_modulename;
	private Spinner sp_moduleid;
	private Button btn_send;
	private Button btn_scan;
	LinearLayout ll_moduleid;
...
//	public final static int  MSG_UPDATE_TEXTVIEW_PC				= 21;
	...
	public final static int  MSG_UPDATE_IP_ADDRESS				= 102;
	public final static int  MSG_UPDATE_MODULEID				= 103;
	...
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState, R.layout.activity_dueros, false);
...
//		 iatDialog = new RecognizerDialog(this, mInitListener);
		...
		mSharedPreferences = getSharedPreferences("DUEROS_SETTING" + PubStatus.g_CurUserName, Activity.MODE_PRIVATE);
...
//		}, 2000);
	...
	private void showValue() {
		...
		if (map_Key_ModuleID.containsKey(key)) {
			...
			SmartPlugDefine module = getSmartPlug(moduleID);
			if (module != null) {
				tv_modulename.setText(module.mPlugName);
				tv_moduleid.setText(module.mPlugId);
			}
		}
	...
	private void acquire() {
		if (null == wakeLock) {
			...
			wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK
					| PowerManager.ON_AFTER_RELEASE, DuerOSActivity.class.getName());
			if (null != wakeLock) {
				wakeLock.acquire();
			}
		}
...
//				showTip("语法构建成功：" + grammarId);
	...
	private void initLayout() {
        et_ip_address = (TextView)findViewById(R.id.et_ip_address);
        tv_moduleid = (TextView)findViewById(R.id.tv_moduleid);
        tv_modulename = (TextView)findViewById(R.id.tv_modulename);
        sp_moduleid = (Spinner)findViewById(R.id.sp_moduleid);
        
        ll_moduleid = (LinearLayout)findViewById(R.id.ll_moduleid);
        ll_moduleid.setOnClickListener(this);
	...
	public void onClick(View view) {
		switch (view.getId()) {
		case R.id.ll_moduleid:
			showAllModules(MSG_UPDATE_MODULEID);
		case R.id.btn_send:
			// do nothing...
			break;
		case R.id.btn_scan:
...
//			}
	...
	private void saveModuleIDToPi(String command) {
		ip_address = et_ip_address.getText().toString();
//		module_id = tv_moduleid.getText().toString();
//		
//		String command = "ModuleID," + module_id + "#";
		new SendMsgUDP(command).run();		
	}
	...
	private void showAllModules(final int cmd_type) {
		ActionSheetDialog dlg = new ActionSheetDialog(DuerOSActivity.this)
		.builder()
		.setTitle(DuerOSActivity.this.getString(R.string.smartplug_oper_selectmodule_title))
		.setCancelable(true)
	...
	private void ResetWakeup() {
		...
		if (mIvw != null) {
			...
			String resPath = ResourceUtil.generateResourcePath(DuerOSActivity.this,
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_UPDATE_SHOW_INFO:
				Toast.makeText(DuerOSActivity.this, (String) msg.obj,
			...
			case MSG_NOTIFY_FAIL:
				Toast.makeText(DuerOSActivity.this, "发送失败", Toast.LENGTH_SHORT).show();
				break;
			case MSG_NOTIFY_NETWORK_FAIL:
				Toast.makeText(DuerOSActivity.this, "网络错误", Toast.LENGTH_SHORT).show();
				break;
			case MSG_NOTIFY_INIT:
				Toast.makeText(DuerOSActivity.this, "未知的初始化消息", Toast.LENGTH_SHORT)
						.show();
				break;
			case MSG_SHOW_MESSAGE:
				String str = (String) msg.obj;
				Toast.makeText(DuerOSActivity.this, str, Toast.LENGTH_SHORT).show();
				break;
			case MSG_UPDATE_TEXTVIEW:
				if (cmd_type == MSG_UPDATE_ZHUWO_KONGTIAO) {
					...
					commandpi = "ZHUWO_KONGTIAO," + moduleids.get(index).mPlugId + "," + kongtiao_type + "#";
					...
					commandpi = "CIWO_KONGTIAO," + moduleids.get(index).mPlugId + "," + kongtiao_type + "#";
					...
					commandpi = "SHUFANG_KONGTIAO," + moduleids.get(index).mPlugId + "," + kongtiao_type + "#";
					...
					commandpi = "KETING_KONGTIAO," + moduleids.get(index).mPlugId + "," + kongtiao_type + "#";
					...
					commandpi = "ZHUWO_CURTAIN," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "CIWO_CURTAIN," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "SHUFANG_CURTAIN," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "KETING_CURTAIN," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "ZHUWO_SMARTPLUG," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "CIWO_SMARTPLUG," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "SHUFANG_SMARTPLUG," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "KETING_SMARTPLUG," + moduleids.get(index).mPlugId + "#";
					...
					commandpi = "KETING_PC," + PC_moduleids.get(index) + "," + PC_moduleidmacs.get(index) + "," + moduleMac.mMAC + "#";
					
				} else if (cmd_type == MSG_UPDATE_MODULEID) {
					key = "树莓派";
					tv_modulename.setText(moduleids.get(index).mPlugName);
					tv_moduleid.setText(moduleids.get(index).mPlugId);
					
					map_Key_ModuleID.put(key, moduleids.get(index).mPlugId);
					
					commandpi = "MODULEID," + moduleids.get(index).mPlugId + "#";
				}
				
				if(commandpi.isEmpty() == false) {
					saveModuleIDToPi(commandpi);
				}
			...
			case MSG_SEND_FAIL:
				...
				break;
			case MSG_UPDATE_IP_ADDRESS:
				String ipAddress = (String)msg.obj;
				et_ip_address.setText(ipAddress);
				break;
			default:
				Toast.makeText(DuerOSActivity.this, "不可能的消息", Toast.LENGTH_SHORT)
						.show();
				break;
			}
		}
	   ...
	   public class SendMsgUDP extends Thread
	    {
	        ...
	        {
				try {
					InetAddress remoteInetAddress = InetAddress.getByName(ip_address); // 服务器地址
				    byte[] bytesToSend = command.getBytes();
			
				    DatagramSocket socket = new DatagramSocket();
				    socket.setSoTimeout(5000); 											// 设置阻塞时间
				
				    DatagramPacket sendPacket = new DatagramPacket(bytesToSend, 		// 相当于将发送的信息打包
				        bytesToSend.length, remoteInetAddress, SERVER_PORT);
				
		    		socket.send(sendPacket);
		    		
		    		mHandler.sendEmptyMessage(MSG_SEND_OK);
		...
		private void scanportWithUDPMode_Button() {
			...
			try {
				inetAddress = FucUtil.getBroadcastAddress(this, "255.255.255.255");
				String dest_mask = inetAddress.getHostAddress();
				new scanportWithUDP_Async(dest_mask, UDP_SCANPORT_COMMAND).execute();
		...
		class scanportWithUDP_Async extends AsyncTask<Void, Void, Void> {
			...
			public scanportWithUDP_Async(String remoteAddress, String sendText) {
			...
			protected Void doInBackground(Void... params) {
				...
				try {
					InetAddress remoteInetAddress = InetAddress.getByName(remoteAddress); // 服务器地址
			        ...
			        if (cmds.length >= 2) {
			        	if (cmds[0].equals("IP") == true) {
			        		Message msg = new Message();
			        		msg.what = MSG_UPDATE_IP_ADDRESS;
			        		msg.obj = String.valueOf(cmds[1]);
			        		mHandler.sendMessage(msg);
			        	}
			        }
