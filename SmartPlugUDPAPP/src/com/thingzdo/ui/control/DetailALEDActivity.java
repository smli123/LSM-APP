...
public class DetailALEDActivity extends TitledActivity implements OnClickListener{
	...
	private BroadcastReceiver mDetailRev = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(PubDefine.PLUG_NOTIFY_ONLINE)) {
				if (true == NotifyProcessor.onlineNotify(DetailALEDActivity.this, intent)) {
					updateUI();
				}
			}
			
			if (intent.getAction().equals(PubDefine.PLUG_NOTIFY_POWER)) {
				if (true == NotifyProcessor.powerNotify(DetailALEDActivity.this, intent)) {
					updateUI();
				}
			}
	...
	public void onClick(View view) {
		...
		case R.id.titlebar_rightbutton:
			...
			if (btn_TitleRight.getText().equals(getString(R.string.smartplug_title_plug_detail))) {
				...
				intent.setClass(DetailALEDActivity.this, PlugDetailInfoActivity.class);
			...
			// WiFi直连：“重选”界面
				...
				disconnectSocket();
				Intent intent = new Intent();
				intent.setClass(DetailALEDActivity.this, AddSocketActivity2.class);
				startActivity(intent);
				finish();
			}
			break;
